...
public class JCatalog extends JPanel implements ListSelectionListener, CatalogSelector {
    ...
    public JCatalog(AppView oApp, int iPriceMode) {
        ...
        Image defimg;
        try {
            defimg = ImageIO.read(getClass().getClassLoader().getResourceAsStream("net/adrianromero/images/folder_yellow.png"));               
        } catch (Exception fnfe) {
            defimg = null;
        } 
        tnbcat = new ThumbNailBuilder(32, 32, defimg);
        
        // Load products default thumbnail
        try {
            defimg = ImageIO.read(getClass().getClassLoader().getResourceAsStream("net/adrianromero/images/package.png"));               
        } catch (Exception fnfe) {
            defimg = null;
        }           
    ...
    public void showCatalogPanel(String reference) {
           
        if (reference == null) {
            showRootCategoriesPanel();
        } else {            
            showProductPanel(reference);
        }
    }
    ...
    public void loadCatalog() throws BasicException {
        ...
        // Load all categories.
        java.util.List<CategoryInfo> categories = m_App.lookupDataLogic(SentenceContainer.class).getRootCategories(); 
    ...
    private void selectCategoryPanel(String catid) {
        ...
        try {
            ...
            if (!m_categoriesset.contains(catid)) {
                ...
                // Add subcategories
                java.util.List<CategoryInfo> categories = m_App.lookupDataLogic(SentenceContainer.class).getSubcategories(catid);
                for (CategoryInfo cat : categories) {
                    jcurrTab.addButton(tnbcat.getThumbNail(cat.getImage()), cat.getName(), new SelectedCategory(cat));
                }
                
                // Add products
                java.util.List<ProductInfoExt> products = m_App.lookupDataLogic(SentenceContainer.class).getProductCatalog(catid);
                for (ProductInfoExt prod : products) {
                    jcurrTab.addButton(tnbprodtext.getThumbNail(prod), prod.getName(), new SelectedAction(prod));
                }
            }
    ...
    private void showProductPanel(String reference) {
            
        ProductInfoExt product = m_productsset.get(reference);

        if (product == null) {
            if (m_productsset.containsKey(reference)) {
                ...
                try {
                    ...
                    java.util.List<ProductInfoExt> products = m_App.lookupDataLogic(SentenceContainer.class).getProductComments(reference);

                    if (products.size() == 0) {
                        // no hay productos por tanto lo anado a la de vacios y muestro el panel principal.
                        m_productsset.put(reference, null);
                        if (showingcategory == null) {
                            showRootCategoriesPanel();                         
                        } else {
                            showSubcategoryPanel(showingcategory);
                        ...
                        // Load product panel
                        product = m_App.lookupDataLogic(SentenceContainer.class).getProductInfo2(reference);
                        m_productsset.put(reference, product);

                        JCatalogTab jcurrTab = new JCatalogTab();      
                        m_jProducts.add(jcurrTab, "PRODUCT." + reference);                        
                        ...
                        selectIndicatorPanel(new ImageIcon(tnbprodtext.getThumbNail(product)), product.getName());

                        CardLayout cl = (CardLayout)(m_jProducts.getLayout());
                        cl.show(m_jProducts, "PRODUCT." + reference); 
                    }
    ...
    private void initComponents() {
        ...
        jPanel4.add(m_lblIndicator);
        m_lblIndicator.setBounds(0, 0, 180, 40);
        ...
        m_btnBack.setIcon(new javax.swing.ImageIcon(getClass().getResource("/net/adrianromero/images/3uparrow.png")));
