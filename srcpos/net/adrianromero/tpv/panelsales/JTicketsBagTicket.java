...
//    private Template m_tempTicket;
    ...
    public JTicketsBagTicket(AppView oApp, UserView user, JPanelTicketEdits panelticket) {
    ...
    private void readTicket() {
        ...
        try {
            ...
            TicketInfo ticket = m_App.lookupDataLogic(SentenceContainer.class).loadTicket(ticketid);
            if (ticket != null) {
                m_ticket = ticket;
    ...
    private void printTicket() {
        ...
        if (m_ticket == null) {
            ...
            try {
                ...
                m_TTP.printTicket(script.eval(m_App.lookupDataLogic(DataLogicSystem.class).getResourceAsXML("Printer.TicketPreview")).toString());
    ...
    private void m_jPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jPrintActionPerformed
        ...
        if (m_ticket != null) {
            try {
                ScriptEngine script = ScriptFactory.getScriptEngine(ScriptFactory.VELOCITY);
                script.put("ticket", m_ticket);
                m_TTP2.printTicket(script.eval(m_App.lookupDataLogic(DataLogicSystem.class).getResourceAsXML("Printer.TicketPreview")).toString());
            } catch (ScriptException e) {
